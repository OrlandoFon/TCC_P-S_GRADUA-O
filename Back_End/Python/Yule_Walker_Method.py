...
raw_data = scipy.io.loadmat('/media/ubuntu_storage/Projeto_Aplicado_XP/Dados/diagnostics101_data/bearing1.mat')
#Define as carateriísticas do rolamento
samp_rate = 51.2e3  # Taxa de amostragem (Hz)
rpm = 1200  # Velocidade de rotação do eixo (RPM)
freq = [4.4423, 6.5577, 0.4038, 5.0079]
...
# Separação do sinal discreto
x = raw_data['vib'].flatten()
N = len(x)
temp_kurt = []

# Estimação do modelo de Auto-Regressivo (AR) com equação Yule-Walker e cálculo da kurtosis residual
for p in range(1, max_p + 1):
    if p % 50 == 0:
        print(f'p = {p}')
    #Estima parametros AR com equação de Yule-Walker    
    rho, sigma = yule_walker(x, order=p)
    
    # Construção do modelo AR com as dimensões corretas
    X = np.zeros((N, p))
    for i in range(1, p + 1):
        X[i:, i - 1] = x[:N - i]
    
    # Faz com que rho[1:] tenha mesma dimensão de X
    if len(rho[1:]) == p:
        xp = -X @ rho[1:]  # Predição do modelo AR (rho[1:] vai "pular" o termo interceptado)
        e = x - xp  # Erro
        temp_kurt.append(kurtosis(e[p:], fisher=False))  # Calcula e armazena o valor da kurtosis
    else:
        temp_kurt.append(np.nan)  # Adiciona valor nulo ao array que armazena a kurtosis se as dimensões não forem iguais
...
freq_rang = [1.8e4, 2.3e4]  # Banda ajustada conforme exemplo do artigo
